...
import { EmailServer } from './email-server.interface';
 ...
 */
 async sendMailSandBox(email: CreateEmailServerDto): Promise<void> {
    const templeteFile = path.join(__dirname, '../../src/email-server/template/notification.pug'); 
    ...
    const render = this._bodytemplete(templeteFile, data);
