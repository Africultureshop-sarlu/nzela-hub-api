...
export class TypeEstablishmentController {
  constructor(private typeEstablishmentService: TypeEstablishmentService) {}

  @Get()
  async getTypeEstablishments(@Res() res: Response): Promise<any> {
    try {
      const typeEstablishments =
        await this.typeEstablishmentService.getTypeEstablishments();
  ...
  async createTypeEstablishment(
    ...
    try {
      const typeEstablishmentCreated =
        await this.typeEstablishmentService.createTypeEstablishments(
          addTypeEstablishmentDto,
        );
